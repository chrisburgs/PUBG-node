  ...
  try {
    ...
    matchesReduced = matches.map(match => 
      ({
        ...
        stats: match.included.map(players => 
          players.type == 'participant' && players
        ).filter(player => player && player.attributes.stats.name == req.query.playerName)
      })
    )
    store.set(`playerData-${req.query.playerName}`, {...storeData, matchDataArray: matchesReduced})
    res.setHeader('Content-Type', 'application/vnd.api+json');
